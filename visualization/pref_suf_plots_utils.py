...
def create_dataframes(prefix_dicts, suffix_dicts, string_list_models):
    ...
    df_suffix_mae : pd.DataFrame
        ...
        MAE for each model.
    """
    # Collect all unique prefix and suffix lengths from all dictionaries
    all_prefix_lengths = sorted(list(set(k for d in prefix_dicts for k in d.keys())))
    all_suffix_lengths = sorted(list(set(k for d in suffix_dicts for k in d.keys())))

    # Use the union of all lengths for consistent dataframes, but limit to first 14 values
    prefix_lengths = all_prefix_lengths
    suffix_lengths = all_suffix_lengths

    # For instance counts, we can take from the first dict that has the key, or sum them up if appropriate.
    # For now, let's assume instance counts are consistent enough or take from the first dict.
    # If not, this part might need more sophisticated handling (e.g., summing counts).
    # For simplicity, we'll use the instance counts from the first dictionary for now.
    # If a prefix length is not in the first dict, its instance count will be 0.
    prefix_instance_counts = [prefix_dicts[0].get(k, (None, None, 0))[2] for k in prefix_lengths]
    suffix_instance_counts = [suffix_dicts[0].get(k, (None, None, 0))[2] for k in suffix_lengths]

    # Safely extract DLS and MAE, using None for missing keys
    prefix_dls = {f'{string_list_models[i]}_dls': [d.get(k, (None, None, None))[0] for k in prefix_lengths] for i, d in enumerate(prefix_dicts)}
    prefix_mae = {f'{string_list_models[i]}_mae': [d.get(k, (None, None, None))[1] for k in prefix_lengths] for i, d in enumerate(prefix_dicts)}

    suffix_dls = {f'{string_list_models[i]}_dls': [d.get(k, (None, None, None))[0] for k in suffix_lengths] for i, d in enumerate(suffix_dicts)}
    suffix_mae = {f'{string_list_models[i]}_mae': [d.get(k, (None, None, None))[1] for k in suffix_lengths] for i, d in enumerate(suffix_dicts)}
...
def create_plots_log(pref_suf_dfs, 
    ...
    """
    config_string = {
        'CRTP_LSTM_DA': 'CRTP-LSTM (DA)',
        'CRTP_LSTM_NDA': 'CRTP-LSTM (NDA)',
        'ED_LSTM': 'ED-LSTM',
        'ED_LSTM_DA': 'ED-LSTM (DA)',
        'SEP_LSTM': 'SEP-LSTM',
        'SUTRAN_DA': 'SuTraN (DA)',
        'SUTRAN_NDA': 'SuTraN (NDA)',
        'Enhanced_SUTRAN_DA': 'Enhanced SuTraN (DA)',
        'Mamba_DA': 'Mamba (DA)',
    }

    config_styles = {
        'CRTP_LSTM_DA': ('#1f77b4', '-'),
        'CRTP_LSTM_NDA': ('#1f77b4', '--'),
        'ED_LSTM': ('#ff7f0e', '-'),
        'ED_LSTM_DA': ('#ff7f0e', '--'),
        'SEP_LSTM': ('#d62728', '-'),
        'SUTRAN_DA': ('#2ca02c', '-'),
        'SUTRAN_NDA': ('#2ca02c', '--'),
        'Enhanced_SUTRAN_DA': ('#8c564b', '-'),
        'Mamba_DA': ('#e377c2', '-'),
    }
    ...
    if include_legend:
        ...
        all_handles = []
        all_labels = []
        # Manually create legend handles and labels based on configs
        for config_key in configs:
            if config_key in config_string and config_key in config_styles:
                label_text = config_string[config_key]
                color, linestyle = config_styles[config_key]
                # Create a dummy line for the legend
                handle = mlines.Line2D([], [], color=color, linestyle=linestyle, label=label_text)
                if label_text not in all_labels: # Avoid duplicate labels if configs list has duplicates (though unlikely here)
                    all_handles.append(handle)
                    all_labels.append(label_text)
            else:
                print(f"Warning: Config key '{config_key}' not found in config_string or config_styles.")

        # Create a single, common legend at the bottom of the figure
        if all_handles and all_labels:
            fig.legend(all_handles, all_labels, loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol=min(6, len(all_handles)), title='Models')
        else:
            print("Warning: No handles or labels could be generated for the legend.")

    fig.tight_layout()
    # plt.show() # Moved after savefig or handled by calling function
    return fig
