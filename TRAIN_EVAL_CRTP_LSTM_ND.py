...
if __name__ == "__main__":
    # 指定要训练和评估的数据集的名称
    # 这应该与你运行 create_BPIC17_DR_data.py 时使用的 log_name 一致
    # 也就是 'BPIC_17_DR'
    dataset_log_name = 'BPIC_17'  

    print(f"Starting training and evaluation for dataset: {dataset_log_name}")
    
    # 调用主函数，传入数据集名称和 tss_index
    # For NDA, tss is expected to be at index 0 and tsp at index 1 among numerical prefix features.
    train_eval(log_name=dataset_log_name, tss_index=1)
    
    print(f"Finished training and evaluation for dataset: {dataset_log_name}")
    print(f"Results saved in subdirectory: {os.path.join(dataset_log_name, 'SUTRAN_DA_results')}") 
    print(f"Final test set metrics saved in: {os.path.join(dataset_log_name, 'SUTRAN_DA_results', 'TEST_SET_RESULTS')}")